...
import nl.tacticaldev.ddgvanish.exception.InvalidWorldException;
import org.bukkit.*;
...
public class VanishConfig {
    ...
    public VanishConfig(File dir, String name, boolean def) {
        this.folder = dir;
        if (name == null) {
    ...
    public VanishConfig(String name, boolean isDef) {
        this(DDGVanish.getInstance().getDataFolder(), name, isDef);
    }

    public VanishConfig(String name) {
        this(DDGVanish.getInstance().getDataFolder(), name, true);
    }

    public VanishConfig(File file) {
        this(file, null, true);
    }

    public VanishConfig(File dir, String name) {
        this(dir, name, true);
    }
    ...
    public boolean reload() {
        if (!this.folder.exists()) {
            try {
                if (this.folder.mkdir()) {
                    DDGVanish.getInstance().getLogger().log(Level.INFO, "§a - §9Created a new configuration file!");
                } else {
                    DDGVanish.getInstance().getLogger().log(Level.INFO, "§4- §cUnable to create folder " + this.folder.getName());
                }
            } catch (Exception e) {
                DDGVanish.getInstance().getLogger().log(Level.INFO, "Failed to reload " + this.fileName + " config.");
                return false;
            }
        }
        ...
        if (this.def) {
            InputStream defaultConfigStream = DDGVanish.getInstance().getResource(this.fileName);
...
//    }
    ...
    public boolean save() {
        ...
        try {
            ...
            DDGVanish.getInstance().getLogger().log(Level.INFO, ("Failed to save config to " + this.configFile.getName()));
            e.printStackTrace();
            return false;
        }
    }
    ...
    private void saveDefaultConfig() {
        if (this.configFile == null)
            this.configFile = new File(DDGVanish.getInstance().getDataFolder(), this.fileName);

        if (!this.configFile.exists()) DDGVanish.getInstance().saveResource(this.fileName, false);
    }
}