package nl.tacticaldev.ddgvanish.config;
...
public class VanishConf extends YamlConfiguration {
    protected static final Logger LOGGER = Logger.getLogger("DDGVanish");
    protected final File configFile;
    protected String templateName = null;
    protected static final Charset UTF8 = StandardCharsets.UTF_8;
    private Class<?> resourceClass = VanishConf.class;
    private static final ExecutorService EXECUTOR_SERVICE = Executors.newSingleThreadExecutor();
    private final AtomicInteger pendingDiskWrites = new AtomicInteger(0);
    private final AtomicBoolean transaction = new AtomicBoolean(false);

    public VanishConf(final File configFile) {
        super();
    ...
    public synchronized void load() {
        ...
        if (!configFile.getParentFile().exists()) {
            if (!configFile.getParentFile().mkdirs()) {
                LOGGER.log(Level.SEVERE, "Failed to create config {0}", configFile.toString());
            }
        }
        ...
        if (!configFile.exists()) {
            if (legacyFileExists()) {
                ...
                LOGGER.log(Level.INFO, "Creating config from template {0}", configFile.toString());
                createFromTemplate();
            } else {
                return;
            }
    ...
    private void createFromTemplate() {
        ...
        try {
            ...
            if (istr == null) {
                LOGGER.log(Level.SEVERE, "Couldn't find template {0}", templateName);
                return;
            }
            ...
            LOGGER.log(Level.SEVERE, "Failed to write config {0}", configFile.toString());
        } finally {
            try {
                if (istr != null) {
                    istr.close();
                ...
                Logger.getLogger(VanishConf.class.getName()).log(Level.SEVERE, null, ex);
            }
            try {
                if (ostr != null) {
                    ostr.close();
                ...
                LOGGER.log(Level.SEVERE, "Failed to close config {0}", configFile.toString());
            }
        }
    }
        ...
        public void run() {
            ...
            synchronized (configFile) {
                ...
                try {
                    ...
                    if (!configFile.exists()) {
                        try {
                            LOGGER.log(Level.INFO, "Creating empty config {0}", configFile.toString());
